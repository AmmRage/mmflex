...
namespace MMF.Model.Other
{
    public class DebugDotManager
    {
        ...
        public void Draw(List<Vector3> positions,Vector4 color)
        {
            ...
            this.Effect.GetVariableBySemantic("COLOR").AsVector().Set(color);
            for (int i = 0; i < positions.Count; i++)
            {
                ...
                Context.InputAssembler.SetVertexBuffers(0, new VertexBufferBinding(this.VertexBuffer, DebugDotInputLayout.SizeInBytes, 0));
                Context.InputAssembler.InputLayout = this.VertexLayout;
                Context.InputAssembler.PrimitiveTopology = PrimitiveTopology.TriangleList;
                this.RenderPass.Apply(Context);
                Context.Draw(6, 0);
            }
        }
}
