...
namespace MMF.Bone
{
    public class ManualTransformUpdater:ITransformUpdater
    {
        ...
        {
            var boneDictionary = this.model.Skinning.BoneDictionary;
            var morphManager = this.model.Morphmanager;
            foreach (var boneTransformer in this.updaters)
            {
            ...
            foreach (var morphTransformer in this.morphUpdaters)
            {
        ...
        public BoneTransformer getBoneTransformer(string boneName)
        {
            ...
            if(!this.model.Skinning.BoneDictionary.ContainsKey(boneName))throw new InvalidOperationException("そのような名前のボーンは存在しません。");
            if (this.updaters.ContainsKey(boneName)) return this.updaters[boneName];
            else transformer=new BoneTransformer(boneName,Quaternion.Identity,Vector3.Zero);
            this.updaters.Add(transformer.BoneName,transformer);
            return transformer;
        }
        ...
        public MorphTransformer getMorphTransformer(string morphName)
        {
            ...
            if (this.morphUpdaters.ContainsKey(morphName)) return this.morphUpdaters[morphName];
            else transformer=new MorphTransformer(morphName);
            this.morphUpdaters.Add(transformer.MorphName,transformer);
            return transformer;
        }
    }
