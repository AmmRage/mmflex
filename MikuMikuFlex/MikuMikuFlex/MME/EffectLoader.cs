...
namespace MMF.MME
{
    ...
    public class EffectLoader : IDisposable
    {
        ...
        public ShaderBytecode GetShaderBytecode(string fileName, Stream fileStream)
        {
            ...
            using (
                SQLiteCommand getBlobCommand = new SQLiteCommand(string.Format(getBlobQuery, fileName, hashStr), this.Connection))
            using (SQLiteDataReader blobReader = getBlobCommand.ExecuteReader())
            {
                if (blobReader.Read()) //Retrieved from the database file name with the hash if (time without updating the file the same name)
                {
                            ...
                            while (fileNameReader.Read())
                            {
                                using (
                                    SQLiteCommand deleteSameNameCommand =
                                        new SQLiteCommand(string.Format(deleteByIdQuery, fileNameReader.GetInt32(0)), this.Connection))
                                {
