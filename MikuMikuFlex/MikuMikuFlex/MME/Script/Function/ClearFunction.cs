...
namespace MMF.MME.Script.Function
{
    internal class ClearFunction:FunctionBase
    {
        ...
        {
            if (this.isClearDepth)
            {
                this.context.DeviceManager.Context.ClearDepthStencilView(this.context.CurrentRenderDepthStencilTarget,DepthStencilClearFlags.Depth|DepthStencilClearFlags.Stencil, this.context.CurrentClearDepth,0 );
            }
            else
            {
                this.context.DeviceManager.Context.ClearRenderTargetView(this.context.CurrentRenderColorTargets[this.index], this.context.CurrentClearColor);
            }
        }
    }
}
