...
namespace MMF.MME.Script
{
    ...
    MMEEffectPass pass)
        ...
        {
            this.ParsedExecuters=new List<FunctionBase>();
            if(string.IsNullOrWhiteSpace(this.ScriptCode))return;
            string[] splittedFunctions = this.ScriptCode.Split(';');//;で分割し1命令ごとにする
            foreach (string function in splittedFunctions)
            {
                ...
                {
                    this.ParsedExecuters.Add(ScriptFunctions[segments[0]].GetExecuterInstance(index,segments[1],context,this,manager, technique, pass));
                }
            }
        }
        ...
        {
            for (this.CurrentExecuter=0; this.CurrentExecuter < this.ParsedExecuters.Count; this.ParsedExecuters[this.CurrentExecuter].Increment(this))
            {
                this.ParsedExecuters[this.CurrentExecuter].Execute(ipmxSubset,drawAction);
            }
        }
