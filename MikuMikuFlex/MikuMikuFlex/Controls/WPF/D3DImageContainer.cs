...
{
    /// <summary>
    ///     For contents of the texture visible in the WPF class
    /// </summary>
    public class D3DImageContainer : D3DImage, IDisposable
    {
        ...
        public void SetBackBufferSlimDX(Texture2D texture)
        {
            ...
            if (IsShareable(texture))
            {
                ...
                this.SharedTexture = new Texture(D3DDevice, texture.Description.Width, texture.Description.Height, 1,
                    Usage.RenderTarget, format, Pool.Default, ref handle);

                this.surface = this.SharedTexture.GetSurfaceLevel(0);
                {
                    Lock();
                    SetBackBuffer(D3DResourceType.IDirect3DSurface9, this.surface.ComPointer);
                    Unlock();
                }
            }
        ...
        /// <summary>
        ///     To check whether or not sharing textures
        /// </summary>
        ...
        /// <summary>
        ///     Need and so to initialize the DirectX9。(WPF用)
        /// </summary>
